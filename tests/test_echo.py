...
class TestEcho(unittest.TestCase):
    ...
    def test_parser_namespace(self):
        """Check if parser creates namespace object"""
        result = self.module.create_parser()
        self.assertIsInstance(
            result, argparse.ArgumentParser,
            "create_parser() function is not returning a parser object")
        args = ["-l", "hello world"]
        ns = result.parse_args(args)
        ns_list = [ns.lower, ns.text, ns.title, ns.upper]
        expected_list = [True, 'hello world', False, False]
        self.assertListEqual(ns_list, expected_list)
    ...
    def test_multiple_options(self):
        """Check if multiple arguments are applied correctly"""
        args = ["-ult", 'heLLO woRlD']
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], 'Hello World')

    def test_help_message(self):
        "Check if program output matches expected usage message"
        args = ["-h"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "usage: echo.py [-h] [-u] [-l] [-t] text")
